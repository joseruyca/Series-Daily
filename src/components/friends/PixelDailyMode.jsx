...
export default function PixelDailyMode({ challenges, lang, t }) {
  ...
  const hintText = pickLocalizedValue(challenge, "hint", lang);
  ...
  return (
    <section className="friends-play-card friends-mode-reveal">
      ...
      <form className="friends-reveal-form" onSubmit={handleSubmit}>
        <input type="text" className="friends-reveal-input" placeholder={t.challenge.inputPlaceholder} value={guess} onChange={(event) => setGuess(event.target.value)} disabled={progress.finished} autoComplete="off" spellCheck="false" />
